...
if ($visiteur == 9)
{
    ...
    function main()
    {
        ...
        if ($query != "")
        {
            $and = "AND (UT.pseudo LIKE '%" . mysql_real_escape_string($query) . "%')";
            $url_page = "index.php?file=Admin&amp;page=user&amp;query=" . urlencode($query) . "&amp;orderby=" . urlencode($orderby);
        }
        else
        {
            $url_page = "index.php?file=Admin&amp;page=user&amp;orderby=" . urlencode($orderby);
            $and = "";
        }
    ...
    . "<div class=\"tab-content\" id=\"tab2\"><form method=\"get\" action=\"index.php\">\n"
        ...
        $orderby_val = isset($_REQUEST['orderby']) ? $_REQUEST['orderby'] : '';
        if ($orderby_val == "date")
        {
            $order_by = "UT.date DESC";
        }
        else if ($orderby_val == "level")
        {
            $order_by = "UT.niveau DESC, UT.date DESC";
        }
        else if ($orderby_val == "last_date")
        {
            $order_by = "ST.date DESC";
        }
        else if ($orderby_val == "pseudo")
        {
            $order_by = "UT.pseudo";
        }
        else
        {
            $order_by = "UT.date DESC"; // Default
        }
        ...
        $orderby_display = isset($_REQUEST['orderby']) ? $_REQUEST['orderby'] : '';
        if ($orderby_display == "level" || !$orderby_display)
        {
            echo "<b>" . _LEVEL . "</b> | ";
        }
        else

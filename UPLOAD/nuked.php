...
$cookie_name = isset($nuked['cookiename']) ? $nuked['cookiename'] : 'nuked';
$user_theme_key = $cookie_name . '_user_theme';
$nuked['user_theme'] = isset($_REQUEST[$user_theme_key]) ? $_REQUEST[$user_theme_key] : '';
...
$user_lang_key = $cookie_name . '_user_langue';
$nuked['user_lang'] = isset($_REQUEST[$user_lang_key]) ? $_REQUEST[$user_lang_key] : '';
...
// Compatibility function for strftime (deprecated in PHP 8.1)
if (!function_exists('nk_strftime')) {
    function nk_strftime($format, $timestamp = null) {
        if ($timestamp === null) {
            $timestamp = time();
        }
        
        // Try to use strftime if available (PHP < 8.1)
        if (function_exists('strftime')) {
            return strftime($format, $timestamp);
        }
        
        // Convert strftime format to date() format for PHP 8.1+
        $format_map = array(
            '%d' => 'd', '%m' => 'm', '%Y' => 'Y', '%y' => 'y',
            '%H' => 'H', '%M' => 'i', '%S' => 's',
            '%a' => 'D', '%A' => 'l', '%b' => 'M', '%B' => 'F',
...
// Compatibility function for utf8_encode (deprecated in PHP 8.2)
if (!function_exists('nk_utf8_encode')) {
    function nk_utf8_encode($string) {
        if (function_exists('utf8_encode')) {
            return utf8_encode($string);
        }
        // Fallback using mb_convert_encoding
...
function nkDate($timestamp, $blok = FALSE) {
    ...
    $isBlok = ($blok === FALSE) ? (isset($nuked['IsBlok']) ? $nuked['IsBlok'] : FALSE) : $blok;
    $format = ($isBlok === TRUE) ? ($language == 'french') ? '%d/%m/%Y' : '%m/%d/%Y' : (isset($nuked['dateformat']) ? $nuked['dateformat'] : '%m/%d/%Y');
    // iconv pour �viter les caract�res sp�ciaux dans la date
    return iconv('UTF-8','ISO-8859-1', nk_strftime($format, $timestamp));
...
function session_open($path, $name){
    ...
    if (!isset($GLOBALS['global']) || !is_array($GLOBALS['global'])) {
        return false; // Database not configured yet
    }
    return true;
}
...
function session_read($id){
    if (!defined('TMPSES_TABLE')) {
        return ''; // Constants not loaded yet, return empty
    }
    
    connect();

    $id_escaped = mysql_real_escape_string($id);
    $sql = mysql_query('SELECT session_vars FROM ' . TMPSES_TABLE . ' WHERE session_id = "' . $id_escaped . '"');
    if($sql && mysql_num_rows($sql) > 0){
        return mysql_result($sql, 0);
    }
    return ''; // Return empty string if no session found
}

// WRITE PHP SESSION
function session_write($id, $data){
    if (!defined('TMPSES_TABLE')) {
        return false; // Constants not loaded yet
    }
    ...
    $sql = mysql_query('INSERT INTO ' . TMPSES_TABLE . ' (session_id, session_start, session_vars) VALUES ("' . $id_escaped . '", ' . time() . ', \'' . $data_escaped . '\')');

    if ($sql === false || mysql_affected_rows() == 0) {
        $sql = mysql_query('UPDATE ' . TMPSES_TABLE . ' SET session_vars = \'' . $data_escaped . '\' WHERE session_id = "' . $id_escaped . '"');
    }
...
function session_delete($id){
    ...
    $id_escaped = mysql_real_escape_string($id);
    $sql = mysql_query('DELETE FROM ' . TMPSES_TABLE . ' WHERE session_id = "' . $id_escaped . '"');

    return $sql !== false;
}

// KILL DEAD SESSION
// Renamed to avoid conflict with PHP 8.0+ built-in session_gc() function
function nk_session_gc($maxlife){
    if (!defined('TMPSES_TABLE')) {
        return true; // Constants not loaded yet, return success
    }
...
// CONFIG PHP SESSION
if(ini_get('session.save_handler') == 'files') session_set_save_handler('session_open', 'session_close', 'session_read', 'session_write', 'session_delete', 'nk_session_gc');
...
function banip() {
    ...
    $user_pseudo = (isset($user) && is_array($user) && isset($user[2])) ? $user[2] : '';
    $where_query = ' WHERE (ip LIKE "%' . $ip_dyn . '%")' . ($user_pseudo ? ' OR pseudo = "' . mysql_real_escape_string($user_pseudo) . '"' : '');
...
function nbvisiteur(){
    ...
    if (isset($user_ip)){
        ...
        if ($query > 0){
            $user_id = (isset($user) && is_array($user) && isset($user[0])) ? mysql_real_escape_string($user[0]) : '0';
            $user_type = (isset($user) && is_array($user) && isset($user[1])) ? mysql_real_escape_string($user[1]) : '0';
            $user_name = (isset($user) && is_array($user) && isset($user[2])) ? mysql_real_escape_string($user[2]) : '';
            if ($user_id != '0'){
                $req = mysql_query("UPDATE " . NBCONNECTE_TABLE . " SET date = '" . $limite . "', type = '" . $user_type . "', IP = '" . mysql_real_escape_string($user_ip) . "', username = '" . $user_name . "' WHERE user_id = '" . $user_id . "'");
            }
            else{
                $req = mysql_query("UPDATE " . NBCONNECTE_TABLE . " SET date = '" . $limite . "', type = '" . $user_type . "', user_id = '" . $user_id . "', username = '" . $user_name . "' WHERE IP = '" . mysql_real_escape_string($user_ip) . "'");
            }
        }
        else{
            ...
            $user_id = (isset($user) && is_array($user) && isset($user[0])) ? mysql_real_escape_string($user[0]) : '0';
            $user_name = (isset($user) && is_array($user) && isset($user[2])) ? mysql_real_escape_string($user[2]) : '';
            $req = mysql_query("INSERT INTO " . NBCONNECTE_TABLE . " ( `IP` , `type` , `date` , `user_id` , `username` ) VALUES ( '" . mysql_real_escape_string($user_ip) . "' , '" . $user_type . "' , '" . $limite . "' , '" . $user_id . "' , '" . $user_name . "' )");
        }
    }
...
function compteur($file){
    $file_name = isset($_GET['file']) ? mysql_real_escape_string($_GET['file']) : '';
    if ($file_name) {
        $upd = mysql_query('UPDATE ' . STATS_TABLE . ' SET count = count + 1 WHERE type = "pages" AND nom = "' . $file_name . '"');
    }
}
...
function UpdateSitmap(){
    ...
    if ($fp !== false){
        ...
        $Sitemap .= "</urlset>\r\n";
        fwrite($fp, chr(0xEF) . chr(0xBB)  . chr(0xBF) . nk_utf8_encode($Sitemap)); //Ajout de la marque d'Octet
        fclose($fp);
    }
}

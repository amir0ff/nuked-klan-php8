...
function affich_block_login($blok){
    ...
    // Fixed: Add isset check for $blok['content'] and handle explode result
    $content = isset($blok['content']) ? $blok['content'] : '';
    $exploded = explode('|', $content);
    list($login, $messpv, $members, $online, $avatar) = array_pad($exploded, 5, '');
    $blok['content'] = '';
	...
	if($login != 'off'){
		if (!$user){
			...
			// Fixed: Add isset checks for $user array
			$user_name = (isset($user) && is_array($user) && isset($user[2])) ? $user[2] : '';
			$user_id = (isset($user) && is_array($user) && isset($user[0])) ? $user[0] : '';
			$blok['content'] = '<div style="text-align: center;">' . _WELCOME . ', <b>' . $user_name . '</b><br /><br />'."\n";
			if ($avatar != 'off'){
				$sql_avatar=mysql_query('SELECT avatar FROM ' . USER_TABLE . ' WHERE id = \'' . mysql_real_escape_string($user_id) . '\' ');
				list($avatar_url) = mysql_fetch_array($sql_avatar);
				if($avatar_url) $blok['content'] .= '<img src="' . $avatar_url . '" style="border:1px ' . $bgcolor3 . ' dashed; width:100px; background:' . $bgcolor1 . '; padding:2px;" alt="' . $user_name . ' avatar" /><br /><br />';
			}
    ...
    // Fixed: Add isset checks for $user array
    if($messpv != 'off' && isset($user) && is_array($user) && isset($user[0]) && $user[0] != ''){
		if ($c > 0) $blok['content'] .= '<hr style="height: 1px;" />'."\n";
	
		$user_id = mysql_real_escape_string($user[0]);
		$sql2 = mysql_query('SELECT mid FROM ' . USERBOX_TABLE . ' WHERE user_for = \'' . $user_id . '\' AND status = 1');
		...
		$user_mess = (isset($user[5])) ? $user[5] : 0;
		if ($user_mess > 0){
			$blok['content'] .= '&nbsp;<b><big></big></b>&nbsp;' . _NOTREAD . ' : <a href="index.php?file=Userbox"><b>' . $user_mess . '</b></a>'."\n";
		}
		else{
			$blok['content'] .= '&nbsp;<b><big></big></b>&nbsp;' . _NOTREAD . ' : <b>' . $user_mess . '</b>'."\n";
		}
	...
	if ($online != 'off'){
    	...
    	$nb = nbvisiteur();
    	// Fixed: Add isset checks for $nb array
    	if (!is_array($nb)) $nb = array(0, 0, 0);

    	$user_online = ''; // Fixed: Initialize $user_online
    	if (isset($nb[1]) && $nb[1] > 0){
		...
		$admin_online = ''; // Fixed: Initialize $admin_online
		if (isset($nb[2]) && $nb[2] > 0){
		...
		$nb_visitors = isset($nb[0]) ? $nb[0] : 0;
		$nb_members = isset($nb[1]) ? $nb[1] : 0;
		$nb_admins = isset($nb[2]) ? $nb[2] : 0;
		$blok['content'] .= '&nbsp;<b><big></big></b>&nbsp;' . _VISITOR;
		if ($nb_visitors > 1) $blok['content'] .= 's';
		$blok['content'] .= ' : <b>' . $nb_visitors . '</b><br />&nbsp;<b><big></big></b>&nbsp;' . _MEMBER;
		if ($nb_members > 1) $blok['content'] .= 's';
		$blok['content'] .= ' : <b>' . $nb_members . '</b>' . $user_list . '<br />&nbsp;<b><big></big></b>&nbsp;' . _ADMIN;
		if ($nb_admins > 1) $blok['content'] .= 's';
		$blok['content'] .= ' : <b>' . $nb_admins . '</b>' . $admin_list . '<br />'."\n";

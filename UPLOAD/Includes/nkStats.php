...
function getStats($nuked)
{
    ...
    $server_ip_check = (strlen($nuked['server_ip'])>0) ? "set" : ""; // Fixed: Initialize $server_ip
    $data['server_game'] = ($server_ip_check == "set" && strlen($nuked['server_game'])>0) ? $nuked['server_game'] : "0";
    ...
    $timediff = isset($timediff) ? $timediff : 0; // Fixed: Initialize $timediff if not set

...
class process {
    ...
    public function saveUserAdmin() {
        ...
        if (! isset($_POST['nickname'], $_POST['password'], $_POST['passwordConfirm'], $_POST['email'])
    ...
    private function _requirements() {
        ...
        foreach ($this->_phpExtension as $extensionName => $requirement) {
            // Special handling for mysql extension - check for mysql_* functions (via mysqli_compat) or extension
            if ($extensionName == 'mysql') {
                // Check if mysql_connect function exists (provided by mysqli_compat.php) or mysql extension is loaded
                if (function_exists('mysql_connect') || extension_loaded($extensionName))
